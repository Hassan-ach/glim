    ...
    try {
        ...
        if (tab && tab.url) {
            ...
            if (isYouTube) {
                ...
                currentUrlSpan.textContent = tab.url;
                showStatus(
                    "Please navigate to a YouTube video page to summarize.",
                    "warning",
                );
                summaryBtn.disabled = true;
                return false;
...
async function checkConfig() {
    try {
        ...
        showStatus(
            "Error loading configuration. Please configure the extension.",
            "error",
        );
        return false;
    }
...
async function handleSummarize() {
    ...
    if (!hasConfig) {
        showStatus(
            "Please configure your API key in the options first.",
            "warning",
        );
        // Open options page
    ...
    try {
        ...
        if (result.success && result.data.html_output) {
            // Create new tab with the HTML content
            const newTab = window.open("about:blank", "_blank");
            newTab.document.open();
            newTab.document.write(result.data.html_output);
            newTab.document.close();
            // localStorage.setItem(
            //     result.data.video_info.title,
            //     result.data.html_output,
            // );
